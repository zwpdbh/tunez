defmodule TunezWeb.Albums.FormLive do
  ...
  def mount(%{"id" => album_id}, _session, socket) do
    ...
    form =
      Tunez.Music.form_to_update_album(album, acstor: socket.assigns.current_user)
  ...
  def mount(%{"artist_id" => artist_id}, _session, socket) do
    ...
    form =
      Tunez.Music.form_to_create_album(artist.id, acstor: socket.assigns.current_user)
      |> AshPhoenix.Form.ensure_can_submit!()
  ...
  def handle_event("save", %{"form" => form_data}, socket) do
    case AshPhoenix.Form.submit(socket.assigns.form, params: form_data) do
      ...
      {:error, from} ->
        socket =
          socket
          |> put_flash(:error, "There was an error creating the album")
          |> assign(:form, from)
